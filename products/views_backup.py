...
@csrf_exempt
@permission_required('products.delete_productimporthistory', raise_exception=True)
...
@permission_required('products.view_productimporthistory', raise_exception=True)
...
def products_data(request):
    try:
        ...
        columns = ['id', 'sku', 'barcode', 'nama_produk', 'variant_produk', 'brand', 'rak_display', 'photo', None, None]
        ...
        for idx, col in enumerate(columns):
            ...
            if col_search:
                ...
                if col == 'rak_display':
                    ...
                    if clean_search:
                        # PERBAIKAN FUNDAMENTAL: Gunakan filtering yang lebih spesifik
                        # Pastikan produk benar-benar ada di rak yang dipilih
                        filter_q &= Q(inventoryrakstock__rak__kode_rak__iexact=clean_search)
                        
                        # Debug: log filter yang diterapkan
                        logger.info(f"Applied rak filter: '{clean_search}' to query")
                else:
...
@permission_required('products.view_productsbundling', raise_exception=True)
...
@permission_required('products.add_productsbundling', raise_exception=True)
...
@permission_required('products.change_productsbundling', raise_exception=True)
...
@permission_required('products.delete_productsbundling', raise_exception=True)
...
@permission_required('products.add_productextrabarcode', raise_exception=True)
...
@require_POST
@permission_required('products.delete_productextrabarcode', raise_exception=True)
...
@require_GET
@permission_required('products.view_productextrabarcode', raise_exception=True)
...
@permission_required('products.view_productaddhistory', raise_exception=True)
...
@permission_required('products.add_rak', raise_exception=True)
...
@permission_required('products.change_rak', raise_exception=True)
...
@permission_required('products.delete_rak', raise_exception=True)
@require_POST # Pastikan hanya menerima POST request
...
@permission_required('products.view_rak', raise_exception=True)
...
@permission_required('products.view_editproductlog', raise_exception=True)
