...
def cancel_order(request):
    try:
        ...
        if not id_pesanan:
            return JsonResponse({'status': 'error', 'message': 'Parameter id_pesanan dibutuhkan.'}, status=400)
        ...
        with transaction.atomic():
            ...
            if first_order.nama_batch:
                try:
                    ...
                    if batch.status_batch == 'closed':
                        return JsonResponse({
                            'status': 'error', 
                            'message': f"Tidak dapat membatalkan order dari batch '{first_order.nama_batch}' karena batch sudah berstatus closed."
                        }, status=403)
                except BatchList.DoesNotExist:
                    pass
            ...
            nama_batch_lama = orders.first().nama_batch
            if nama_batch_lama:
                for order in orders:
            ...
            # Unlink dari batch
            orders.update(nama_batch=None)

        return JsonResponse({'status': 'success', 'message': f'Order {id_pesanan} berhasil dibatalkan, dihapus dari batch, dan stok terkunci dikembalikan.'})
    except Exception as e:
        import traceback
